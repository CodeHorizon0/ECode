...
pub fn render(
    ...
    let total_height =
        total_lines as f32 * row_height + settings.text_top_padding + settings.text_bottom_padding;
...
fn gutter_width(
    ...
    let width = ui.fonts_mut(|fonts| {
        fonts
            .layout_no_wrap(number, font_id.clone(), settings.line_number_color)
            .size()
            .x
    });
...
fn position_to_cursor(
    ...
    if low > 0 && low < char_count {
        ...
        let left_width = ui.fonts_mut(|fonts| {
            fonts
                .layout_no_wrap(
        ...
        let right_width = ui.fonts_mut(|fonts| {
            fonts
                .layout_no_wrap(
